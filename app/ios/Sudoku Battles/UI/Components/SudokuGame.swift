...
struct SudokuGame: View {
    @Binding var board: SudokuBoard
    @State private var states: [SudokuBoard]
    ...
    init(board: Binding<SudokuBoard>) {
        self._board = board
        self.states = [board.wrappedValue]
    }
    ...
    func setCell(x: Int, y: Int, value: Int?) {
        ...
        if let value {
            ...
            for index in 0..<9 {
                ...
                let offsetX = index % 3
                let offsetY = index / 3
                newBoard.disableNote(x: startX + offsetX, y: startY + offsetY, key: value)
            }
        }
        ...
        VStack(spacing: 30) {
                    ...
                    ForEach(0..<9, id: \.self) { row in
                        HStack(spacing: 0) {
                            ForEach(0..<9, id: \.self) { column in
                                let givenValue = board.given(x: column, y: row)
                                let value = givenValue ?? board[(column, row)]
                                let notes = board.notes(x: column, y: row)
            ...
            VStack(spacing: 15) {
                ...
                HStack(spacing: 15) {
                    ForEach(1...5, id: \.self) { number in
                        ...
                        let missing = 9 - board.count(number)
                        VStack(spacing: 0) {
                            Text("\(number)")
            ...
            HStack(spacing: 15) {
                ...
                Button(action: {
                    ...
                    if !states.isEmpty { board = states.last! }
                }) {
                    HStack(spacing: 6) {
                        Text("Undo")
                            .font(.sora(16, .semibold))
