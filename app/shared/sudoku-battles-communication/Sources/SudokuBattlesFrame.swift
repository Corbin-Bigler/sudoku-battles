...
public struct SudokuBattlesFrame: Codable {
    ...
    public var data: Data {
        ...
        let encodedFunction = try! serializer.encode(function)
        let encodedLength = try! serializer.encode(VarInt(payload.count))
        
        var message = Data()
        message.append(contentsOf: encodedFunction)
        message.append(contentsOf: encodedLength)
